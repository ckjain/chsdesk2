...
#
class LiveSearchGridPanel < ::Netzke::Basepack::GridPanel
	config :tbar => ['->', {
		:xtype => 'textfield',
	...
	js_method :init_component, <<-JS
		function() {
			...
			this.down('#live_search_field').on('keydown', function() {this.onLiveSearch(); }, this, { buffer: 500 });
      this.down('#live_search_field').on('blur', function() {this.onLiveSearch(); }, this, { buffer: 500 });
		}
	JS
	...
	js_method :on_live_search, <<-JS
		function() {
			...
			var search_text = this.down('#live_search_field').getValue();
			if (search_text == this.liveSearchBuffer) return;
			this.liveSearchBuffer = search_text;
			this.getStore().proxy.extraParams = ('live_search', search_text);
			this.getStore().load({params:{live_search: search_text,start:0,limit:10}});
		}
	JS
